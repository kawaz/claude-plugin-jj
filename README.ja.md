...
- **jj-expert は独立したエージェント** — 独立したコンテキストで動作し、式言語（Fileset/Revset/Template）や高度な内容の網羅的な知識を内蔵。外部スキルに依存せず完結する。
...
独立したコンテキストで動作する調査専門エージェント。式言語（Fileset/Revset/Template）の詳細リファレンスと高度な操作の知識を内蔵。複雑な問い合わせやトラブルシューティング時に起動される。
